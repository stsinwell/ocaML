...
let sigmoid = {
  (* sigmoid S(x) = 1 / (1 + e^-x) *)
  f = (fun x -> let ones = mat_ones (Mat.dim1 x) 1 in Mat.div ones (Mat.add_const 1.0 (Mat.exp (Mat.neg x))));
...
let softmax = {
  f = (fun x -> Mat.div (Mat.exp x) (mat_const (Mat.dim1 x) 1 (Mat.sum (Mat.exp x))));
